    ...
    module ClassMethods
      def acts_as_tenant(tenant = :account, options = {})
        ...
        default_scope lambda {
          ...
          if ActsAsTenant.current_tenant
            if options.has_key?(:association)
              joins(options[:association]).where("#{options[:association].to_s.pluralize}.#{fkey.to_s} = ?", ActsAsTenant.current_tenant.id )
            else
        ...
        before_validation Proc.new {|m|
          if ActsAsTenant.current_tenant && !options.has_key?(:association)
